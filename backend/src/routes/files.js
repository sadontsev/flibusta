  ...
  try {
    ...
    if (!fileInfo) {
      return res.status(404).json({
        success: false,
        error: 'Book file not found. This book may not be available for download or the file mapping is missing.'
      });
    }
    ...
    try {
      ...
      console.error('ZIP file not found:', error.message);
      return res.status(404).json({
        success: false,
        error: `Book archive not found: ${fileInfo.filename}`,
        bookId: bookId,
        expectedFile: fileInfo.filename
      });
    }
    ...
    if (!bookEntry) {
      ...
      return res.status(404).json({
        success: false,
        error: 'Book file not found in archive'
      });
    }
    ...
    try {
      ...
      try {
        ...
        if (!imageEntry) {
          return res.status(404).json({
            success: false,
            error: 'Author image not found in archive'
          });
        }
        ...
        return res.status(404).json({
          success: false,
          error: 'Author image archive not found'
        });
      }
    }
  } catch (error) {
    ...
    try {
      ...
      try {
        ...
        if (!coverEntry) {
          return res.status(404).json({
            success: false,
            error: 'Book cover not found in archive'
          });
        }
        ...
        return res.status(404).json({
          success: false,
          error: 'Book cover archive not found'
        });
      }
    }
  } catch (error) {
