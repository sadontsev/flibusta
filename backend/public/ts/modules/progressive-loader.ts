...
class ProgressiveLoaderNG {
  ...
  renderAuthorsGrid(authors: any[]) { if (!authors || authors.length === 0) { return `<div class="text-center py-12"><i class="fas fa-user text-gray-500 text-4xl mb-4"></i><h3 class="text-xl font-semibold text-white mb-2">Авторы не найдены</h3><p class="text-gray-400">Попробуйте изменить параметры поиска</p></div>`; } let html = `<div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 xl:grid-cols-6 gap-4" id="authors-grid">`; authors.forEach(author => { html += this.renderAuthorCard(author); }); html += '</div>'; return html; }
  ...
  renderAuthorsSkeleton(count: number) { let html = `<div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 xl:grid-cols-6 gap-4" id="authors-grid">`; for (let i = 0; i < count; i++) { html += `<div class="bg-gray-800 rounded-lg shadow-lg p-4"><div class="flex flex-col items-center text-center"><div class="w-16 h-16 bg-gray-700 loading-shimmer rounded-full mb-3"></div><div class="h-4 bg-gray-700 loading-shimmer rounded w-3/4 mb-2"></div><div class="h-3 bg-gray-700 loading-shimmer rounded w-1/2 mb-3"></div><div class="h-8 bg-gray-700 loading-shimmer rounded w-full"></div></div></div>`; } html += '</div>'; return html; }
